...
class Decoder_1(nn.Module):
    def __init__(self, embed_dims=[64, 128, 320, 512]):
        ...
        self.conv_concat34 = ConvModule(in_channels=(embed_dims[4] + embed_dims[3]),
    ...
    def forward(self, encoder):  # encoder = encoder1, encoder2, cl_encoder3, cl_encoder4
        ...
        decoder2 = self.skip_concat(encoder2, decoder3_up2)
        decoder2 = self.linear_concat23(self.conv_concat23(decoder2))

        #   decoder1
        decoder2_up2 = F.interpolate(decoder2, scale_factor=2, mode='bilinear', align_corners=False)
        decoder1 = self.skip_concat(encoder1, decoder2_up2)
        decoder1 = self.linear_concat12(self.conv_concat12(decoder1))
